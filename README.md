...
**Phyto-Autoscopy 綠色自視症**是 CHLOROCULUS 多視角植物影像擷取與分析裝置的本機控制系統。v0.2 在既有捕捉與硬體控制能力上，加入依 Ruiz-Melero 等人（2024）方法建立的固定俯視／側視植物主要生長尖端追蹤、人工修正與雙鏡頭三維重建流程。
...
```text
data/calibration/  相機內參、畸變係數、雙鏡頭外參、品質報告與預覽
data/analysis/     每個 Analysis Run 的影格配對、偵測、修正、三維軌跡與日誌
```

分析建立流程不區分自動或手動來源：可由 Record 自動帶入 `top`、`side`、`rotating` 目錄，也可直接輸入目錄，掃描確認影像、解析度、配對與環繞角度後使用同一套分析管線。`top_side`（頂+側）保留論文的雙鏡頭方法；`top_side_rotating`（頂+側+環繞）先建立雙鏡頭基準點，再使用旋臂動態外參與局部偵測做穩健多視角精修。無效的環繞觀測只會停用該影格的精修，不會使整筆分析失敗。
