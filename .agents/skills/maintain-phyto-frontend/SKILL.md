...
- Keep the height-limited `可分析紀錄` list inside the first step of `新增分析`, with its records scrolling inside the list. Do not restore a separate available-records dashboard panel. Render `影像目錄` directly in the setup panel without another `InnerPanel`, and route transient validation or scan warnings only through the global notifications interface.
- Do not render separate analysis-method buttons. Infer `top_side_rotating` from an enabled `rotating` source and otherwise use `top_side`. Record auto-fill scans immediately; enabled flags alone do not invalidate the current directory preview, while a path change does.
