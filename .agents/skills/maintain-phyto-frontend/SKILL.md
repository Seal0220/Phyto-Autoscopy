...
- Keep analysis creation as one Record-backed source workflow. Record selection is required and fills one read-only Record root directory; do not restore manual directory entry or separate per-camera paths. The only analysis methods are `fixed` (固定俯視＋側視尖端分析) and `rotating` (每輪旋臂多視角三維重建). Require a successful source preview and valid reusable intrinsics plus an ArUco world reference before submission.
