...
Exposure is automatic but uses backend-owned highlight protection because the camera driver's native auto-exposure behavior meters only a small central area. Every metering region is inset horizontally by 8%; `top` uses the full remaining height, while `side` and `rotating` use only the lower half. The controller evaluates a high luminance percentile and the clipped-pixel ratio inside that region, then adjusts hardware exposure without using a center-weighted average. When one discrete brighter level immediately causes a return to the darker level, retain that brighter value as a temporary highlight guard and require a sustained, meaningfully darker measurement before retrying it; prefer the non-clipped darker level when no discrete value can satisfy both sides of the target range. Do not expose target brightness, baseline exposure, manual exposure range, or an exposure-adjustment setting in camera settings.
