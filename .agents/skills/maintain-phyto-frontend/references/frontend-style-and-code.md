...
Schedule captures use a mode-first hierarchy. A Record directory is named `record_<timestamp>` and contains the complete parent `config.json`, `metadata.csv`, and `record.log.csv`. Each mode is stored below `modes/<ModeName>.<number>/` with its own `config.json`, `metadata.csv`, `mode.log.csv`, and `rounds/` directory. Rotating modes use `rounds/round.01`, `rounds/round.02`, and so on without timestamps; continuous or non-rotating capture uses the sole `rounds/round.00`. Snapshot folders use `snapshot.<number>_<timestamp>`, and image filenames use `<camera>-<mode abbreviation>.<mode number>_r.<round number>_s.<snapshot number>_<timestamp>.png`. Camera captures are persisted as lossless PNG files; MJPEG/JPEG remains a preview transport only. Every filesystem timestamp uses `YYYY.MM.DD-hh.mm.ss.xxxxxx` in the project timezone; serialized CSV and JSON time fields remain ISO 8601. Keep the canonical mode names `ContinuousInterval`, `TimeInterval`, `AngleInterval`, `SpecificAngles`, and `EqualDivisions`, with abbreviations `CI`, `TI`, `AI`, `SA`, and `ED`.
...
Scheduled output uses snapshot-owned camera files rather than camera-owned folders. Round-scoped modes write lossless PNG camera files and reset the snapshot sequence to `01` inside every new round. `continuous_interval` and every fixed dual-camera schedule likewise use lossless PNG files with one schedule-wide sequence. Keep the mode log beside its `snapshots` directory. Every snapshot folder groups the `top`, `side`, and when enabled `rotating` observations from one trigger.
...
Exposure uses a backend-owned fuzzy controller because the camera driver's native auto-exposure behavior meters only a small central area. Every camera has a persisted normalized metering region; without a saved override, the fallback is inset horizontally by 8%, with `top` using the full remaining height and `side`/`rotating` using the lower half. The controller continuously evaluates median luminance, high percentiles, highlight ratio, and clipped-pixel ratio. It adjusts hardware exposure first; only after exposure remains inside the stable control band for the configured consecutive windows, or remains unchanged because it has reached a configured boundary, may it enter the hardware-brightness refinement phase. A failed or unverified exposure command is not stability and must not trigger brightness refinement. During brightness refinement the controller keeps measuring, and a significant lighting change returns that camera to exposure adjustment before brightness is considered again. Camera hardware, stream, metering, exposure, and brightness-control parameters belong to `backend/config/cameras.json`; do not duplicate these values in frontend code. Clamp hardware reads, requested writes, and reported exposure status to the configured `minimum_exposure` through `maximum_exposure` range, currently `-20` through `+100`, including abnormal finite or infinite driver values. Clamp brightness to `0` through `100`, with each camera starting from its own configured default of `50`. Reaching either boundary may temporarily block only that direction; the controller must keep measuring and permit the opposite direction immediately. An exposure write that cannot be verified from a fully dark or unchanged frame follows the same bounded-direction retry behavior. Never permanently disable custom exposure merely because a dark frame could not show a visible response. Do not expose target brightness, baseline exposure, manual exposure range, or an exposure-adjustment setting in camera settings.
...
Each camera preview owns two independent diagnostic icon buttons immediately left of the undistortion action. The crosshair button uses the positioning-crosshair icon and toggles only the small inverse-color center crosshair. The exposure-guide button uses the dashed-rectangle icon and toggles the metering-region outline, bright-emerald overexposed-region boxes, and an `曝光: {value}` badge immediately to the right of FPS. Exposure values and normalized regions come from the existing camera status/WebSocket snapshot; the browser must not decode MJPEG frames or open another polling channel for this display. Use an SVG view box with `preserveAspectRatio="xMidYMid meet"` so raw-frame coordinates align with the `object-contain` image rather than its 16:9 letterbox area. While the exposure guide is open, render a square-cornered region with four corner handles and four edge handles. Dragging updates only the local preview; pointer release saves once through the same-origin BFF and hot-applies the persisted region without reconnecting the camera.
...
- authentication session data stays in signed, `httpOnly`, `SameSite=Strict`, non-secure cookies with a seven-day rolling lifetime; the authenticated shell refreshes it every five minutes and whenever the page becomes visible or the network returns;
