...
Exposure is automatic but uses backend-owned full-frame highlight protection because the camera driver's native auto-exposure behavior meters only a small central area. The controller evaluates a high luminance percentile and the clipped-pixel ratio across the complete frame, then adjusts hardware exposure without using a center-weighted average. Do not expose center metering, target brightness, baseline exposure, manual exposure range, or an exposure-adjustment toggle in camera settings; the frontend presents no exposure controls.
