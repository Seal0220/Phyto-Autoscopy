    ...
    <Panel id="sessions" className="min-[981px]:col-start-1 min-[981px]:row-start-4 [scroll-margin-top:5.6rem] max-[980px]:[scroll-margin-top:8.8rem]" aria-label="工作階段">
      ...
      <div className="overflow-x-auto p-5 max-sm:p-4">
        <table className="w-full min-w-[38rem] border-collapse text-left text-sm">
          <thead><tr className="border-b border-white/10 bg-white/[0.03] text-[11px] font-black tracking-[0.12em] text-neutral-400"><th className="px-4 py-3">工作階段</th><th className="px-4 py-3">狀態</th><th className="px-4 py-3">建立時間</th><th className="px-4 py-3">匯出</th></tr></thead>
          <tbody>
            {sessions.length ? sessions.map((session) => (
              <tr className="border-b border-white/10 text-neutral-200 last:border-b-0" key={session.session_id}>
                <td className="px-4 py-3.5">{session.session_id}</td>
                <td className="px-4 py-3.5">{session.status}</td>
                <td className="px-4 py-3.5">{formatDateTime(session.created_at)}</td>
