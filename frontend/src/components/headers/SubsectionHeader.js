...
export default function SubsectionHeader({
  ...
  return (
    <header
      className={`grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-start gap-3 px-1 ${className || ""}`}
      {...props}
    >
      <div className="min-w-0">
        <h3
          id={titleId}
          className={`m-0 ${TITLE_STYLES[titleMode] || TITLE_STYLES[0]}`}
        >
          {title}
        </h3>
      ...
      {children ? (
        <div className="flex shrink-0 items-center self-start">{children}</div>
      ) : null}
    </header>
  );
}
