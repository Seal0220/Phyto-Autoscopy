...
import ScheduleModeCard from "@/components/schedule/schedule-mode-card";
import Button from "@/components/ui/button";
import SubsectionHeader from "@/components/ui/subsection-header";
import { SCHEDULE_MODE_META, scheduleModeTypeFromLabel } from "@/lib/schedule";

export default function ScheduleModes({ schedule, setSchedule, canEdit }) {
  function addMode() {
    let index = schedule.modes.length + 1;
    while (schedule.modes.some((mode) => mode.id === `mode-${index}`)) index += 1;
    setSchedule((previous) => ({
      ...previous,
      modes: [...previous.modes, { id: `mode-${index}`, type: "seconds_interval", ...SCHEDULE_MODE_META.seconds_interval.initial }],
    }));
  }
  ...
  function updateMode(modeId, patch) {
    setSchedule((previous) => ({
      ...previous,
      modes: previous.modes.map((mode) => (mode.id === modeId ? { ...mode, ...patch } : mode)),
    }));
  }
  ...
  function changeModeType(modeId, label) {
    ...
    setSchedule((previous) => ({
      ...previous,
      modes: previous.modes.map((mode) => (mode.id === modeId ? { id: mode.id, type, ...SCHEDULE_MODE_META[type].initial } : mode)),
    }));
  }
  ...
  return (
    <section className="grid gap-3" aria-labelledby="capture-modes-title">
      <SubsectionHeader
        className="mb-3"
      ...
      >
        <Button className="gap-2" onClick={addMode} disabled={!canEdit || schedule.modes.length >= 20}>
