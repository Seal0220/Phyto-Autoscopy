...
import { DurationInput, NumericInput } from "@/components/ui/input";
...
export default function ScheduleCommonControls({
  ...
  return (
    <section className="grid gap-5 py-1" aria-labelledby="shared-controls-title">
      <SubsectionHeader
      ...
      <div className="grid gap-3 px-1 min-[520px]:grid-cols-2 min-[1180px]:grid-cols-6">
        <DurationInput
          ...
          onValueChange={(value) => setSchedule((previous) => ({ ...previous, [durationKey]: value }))}
          {...durationProps}
          required
        />
        {SCHEDULE_COMMON_FIELDS.map(([key, label, props]) => (
          <NumericInput
            ...
            label={label}
            value={schedule[key]}
            onValueChange={(value) => setSchedule((previous) => ({ ...previous, [key]: value }))}
            {...props}
            required
          />
        ))}
