...
export function durationParts(value, unit) {
  const totalSeconds = Math.max(0, Number(value) || 0) * (DURATION_UNIT_SECONDS[unit] || DURATION_UNIT_SECONDS.seconds);
...
export function durationValue(parts, unit) {
  const totalSeconds = parts.days * 86400 + parts.hours * 3600 + parts.minutes * 60 + parts.seconds;
  const rawValue = totalSeconds / (DURATION_UNIT_SECONDS[unit] || DURATION_UNIT_SECONDS.seconds);
