...
export default function History({
  ...
  return (
    <section
      className="grid max-h-[min(32rem,calc(100vh-8rem))] w-full grid-rows-[auto_minmax(0,1fr)] overflow-hidden rounded-2xl border border-white/15 bg-[#08140f]/95 text-slate-100 shadow-2xl backdrop-blur-2xl"
      aria-label="歷史訊息"
    >
      <header className="flex min-h-13 items-center gap-3 border-b border-white/10 px-4 py-3">
        ...
        <button
          className="grid size-7 cursor-pointer place-items-center rounded-md text-neutral-400 transition-colors hover:bg-white/10 hover:text-white focus-visible:outline-2 focus-visible:outline-emerald-300"
          type="button"
          aria-label="收合歷史訊息"
          onClick={onClose}
        >
